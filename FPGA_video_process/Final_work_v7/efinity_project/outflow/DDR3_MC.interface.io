# Efinity IO Placement 
# Interface File: F:/FPGA_PRJ/YLS/inf_riscv_interconnect_v6_2_test/efinity_project/outflow/DDR3_MC.interface.csv
# Version:        2023.1.150.1.5
# Date:           Jun 23 2023

# Copyright (C) 2013 - 2023  Inc. All rights reserved.


# pin name                        	x	y	z
# --------                        	----	----	---
DDR3_PLL_LOCK                     	0	5	1
DDR3_PLL_RSTN                     	0	6	2
shift[0]                          	0	7	0
shift[1]                          	0	7	2
shift[2]                          	0	8	0
shift_sel[0]                      	0	8	2
shift_sel[1]                      	0	9	0
shift_sel[2]                      	0	9	2
shift_sel[3]                      	0	10	0
shift_sel[4]                      	0	10	2
shift_ena                         	0	11	0
jtag_inst1_SEL                    	0	20	1
jtag_inst1_TDI                    	0	21	1
jtag_inst1_TCK                    	0	22	1
jtag_inst1_TDO                    	0	23	0
jtag_inst1_SHIFT                  	0	23	3
jtag_inst1_CAPTURE                	0	24	3
system_spi_0_io_ss                	0	30	0
sys_clk~CLKOUT~1~30               	0	30	6
sys_clk~CLKOUT~1~31               	0	31	6
system_spi_0_io_sclk_write        	0	32	2
sys_clk~CLKOUT~1~59               	0	59	6
system_spi_0_io_data_0_write      	0	60	0
sys_clk~CLKOUT~1~60               	0	60	6
sys_clk~CLKOUT~1~61               	0	61	6
system_spi_0_io_data_1_write      	0	62	2
sys_clk~CLKOUT~1~62               	0	62	6
system_spi_0_io_data_0_writeEnable	0	63	0
system_spi_0_io_data_1_writeEnable	0	63	2
system_spi_0_io_data_0_read       	0	64	3
system_spi_0_io_data_1_read       	0	65	1
hdmi_tx_slow_clk                  	0	159	1
pll_inst4_CLKOUT0                 	0	162	1
hdmi_tx_fast_clk                  	0	163	1
clk_25m                           	0	164	1
hdmi_tx_clk                       	0	296	3
hdmi_tx_pll_RSTN                  	0	302	2
hdmi_tx_pll_LOCKED                	0	312	1
system_uart_0_io_rxd              	0	314	3
inf_in                            	0	317	3
beep                              	0	321	0
o_dq_lo[14]                       	3	0	2
o_dq_hi[14]                       	4	0	0
o_dq_lo[15]                       	5	0	2
o_dq_hi[15]                       	6	0	2
o_dq_oe[14]                       	7	0	0
i_dq_lo[14]                       	7	0	1
o_dq_oe[15]                       	7	0	2
i_dq_lo[15]                       	7	0	3
i_dq_hi[14]                       	8	0	1
i_dq_hi[15]                       	8	0	3
tac_clk~CLKOUT~11~1               	11	0	6
tac_clk~CLKOUT~12~1               	12	0	6
twd_clk~CLKOUT~13~1               	13	0	6
o_dq_lo[10]                       	14	0	2
twd_clk~CLKOUT~14~1               	14	0	6
o_dq_hi[10]                       	15	0	0
o_dq_lo[12]                       	16	0	2
o_dq_hi[12]                       	17	0	2
o_dq_oe[10]                       	18	0	0
i_dq_lo[10]                       	18	0	1
o_dq_oe[12]                       	18	0	2
i_dq_lo[12]                       	18	0	3
clk_25m~CLKOUT~18~1               	18	0	6
i_dq_hi[10]                       	19	0	1
i_dq_hi[12]                       	19	0	3
DDR3_PLL_CLKOUT4~CLKOUT~19~1      	19	0	6
tac_clk~CLKOUT~22~1               	22	0	6
tac_clk~CLKOUT~23~1               	23	0	6
twd_clk~CLKOUT~24~1               	24	0	6
twd_clk~CLKOUT~25~1               	25	0	6
o_dqs_lo[1]                       	26	0	0
o_dqs_hi[1]                       	26	0	2
o_dqs_oe[1]                       	29	0	2
i_dqs_lo[1]                       	29	0	3
o_dqs_n_oe[1]                     	30	0	0
i_dqs_hi[1]                       	30	0	3
tac_clk~CLKOUT~30~1               	30	0	6
tdqss_clk~CLKOUT~32~1             	32	0	6
tdqss_clk~CLKOUT~33~1             	33	0	6
o_dq_lo[13]                       	37	0	2
o_dq_hi[13]                       	38	0	0
tac_clk~CLKOUT~38~1               	38	0	6
o_dq_lo[9]                        	39	0	2
tac_clk~CLKOUT~39~1               	39	0	6
o_dq_hi[9]                        	40	0	2
twd_clk~CLKOUT~40~1               	40	0	6
o_dq_oe[13]                       	41	0	0
i_dq_lo[13]                       	41	0	1
o_dq_oe[9]                        	41	0	2
i_dq_lo[9]                        	41	0	3
twd_clk~CLKOUT~41~1               	41	0	6
i_dq_hi[13]                       	42	0	1
i_dq_hi[9]                        	42	0	3
tdqss_clk~CLKOUT~50~1             	50	0	6
addr[3]                           	51	0	2
odt                               	71	0	0
ba[0]                             	73	0	2
tdqss_clk~CLKOUT~73~1             	73	0	6
tdqss_clk~CLKOUT~74~1             	74	0	6
o_dm_lo[1]                        	81	0	2
o_dm_hi[1]                        	82	0	0
tac_clk~CLKOUT~82~1               	82	0	6
o_dq_lo[8]                        	83	0	2
twd_clk~CLKOUT~83~1               	83	0	6
o_dq_hi[8]                        	84	0	2
twd_clk~CLKOUT~84~1               	84	0	6
o_dq_oe[8]                        	85	0	2
i_dq_lo[8]                        	85	0	3
i_dq_hi[8]                        	86	0	3
o_dq_lo[11]                       	92	0	0
o_dq_hi[11]                       	92	0	2
tac_clk~CLKOUT~92~1               	92	0	6
twd_clk~CLKOUT~94~1               	94	0	6
cs                                	95	0	0
o_dq_oe[11]                       	95	0	2
tdqss_clk~CLKOUT~95~1             	95	0	6
i_dq_lo[11]                       	96	0	1
i_dq_hi[11]                       	97	0	1
DDR3_PLL_CLKOUT4                  	104	0	1
twd_clk                           	107	0	1
hdmi_rx_fast_clk                  	107	323	1
tac_clk                           	108	0	1
osc_clk                           	108	323	1
tdqss_clk                         	109	0	1
sys_clk                           	110	0	1
clk_10m                           	111	0	1
core_clk                          	112	0	1
hdmi_rx_slow_clk                  	112	323	1
cke                               	123	0	2
tdqss_clk~CLKOUT~123~1            	123	0	6
o_dm_lo[0]                        	131	0	0
o_dm_hi[0]                        	131	0	2
twd_clk~CLKOUT~131~1              	131	0	6
tdqss_clk~CLKOUT~132~1            	132	0	6
ba[1]                             	134	0	0
addr[5]                           	142	0	0
tdqss_clk~CLKOUT~142~1            	142	0	6
tdqss_clk~CLKOUT~143~1            	143	0	6
addr[15]                          	144	0	2
tac_clk~CLKOUT~148~1              	148	0	6
tac_clk~CLKOUT~149~1              	149	0	6
twd_clk~CLKOUT~150~1              	150	0	6
twd_clk~CLKOUT~151~1              	151	0	6
o_dq_lo[0]                        	152	0	2
o_dq_hi[0]                        	153	0	0
o_dq_lo[3]                        	154	0	2
o_dq_hi[3]                        	155	0	2
o_dq_oe[0]                        	156	0	0
i_dq_lo[0]                        	156	0	1
o_dq_oe[3]                        	156	0	2
i_dq_lo[3]                        	156	0	3
tac_clk~CLKOUT~156~1              	156	0	6
i_dq_hi[0]                        	157	0	1
i_dq_hi[3]                        	157	0	3
tac_clk~CLKOUT~157~1              	157	0	6
twd_clk~CLKOUT~158~1              	158	0	6
twd_clk~CLKOUT~159~1              	159	0	6
o_dq_lo[5]                        	163	0	2
o_dq_hi[5]                        	164	0	0
tac_clk~CLKOUT~164~1              	164	0	6
o_dq_lo[1]                        	165	0	2
tac_clk~CLKOUT~165~1              	165	0	6
o_dq_hi[1]                        	166	0	2
twd_clk~CLKOUT~166~1              	166	0	6
o_dq_oe[5]                        	167	0	0
i_dq_lo[5]                        	167	0	1
o_dq_oe[1]                        	167	0	2
i_dq_lo[1]                        	167	0	3
twd_clk~CLKOUT~167~1              	167	0	6
i_dq_hi[5]                        	168	0	1
i_dq_hi[1]                        	168	0	3
tac_clk~CLKOUT~172~1              	172	0	6
o_dq_lo[2]                        	174	0	2
tdqss_clk~CLKOUT~174~1            	174	0	6
o_dq_hi[2]                        	175	0	0
tdqss_clk~CLKOUT~175~1            	175	0	6
o_dq_lo[6]                        	176	0	2
o_dq_hi[6]                        	177	0	2
o_dq_oe[2]                        	178	0	0
i_dq_lo[2]                        	178	0	1
o_dq_oe[6]                        	178	0	2
i_dq_lo[6]                        	178	0	3
i_dq_hi[2]                        	179	0	1
i_dq_hi[6]                        	179	0	3
o_dqs_lo[0]                       	186	0	0
o_dqs_hi[0]                       	186	0	2
tac_clk~CLKOUT~188~1              	188	0	6
o_dqs_oe[0]                       	189	0	2
i_dqs_lo[0]                       	189	0	3
tac_clk~CLKOUT~189~1              	189	0	6
osc_en                            	189	323	0
o_dqs_n_oe[0]                     	190	0	0
i_dqs_hi[0]                       	190	0	3
twd_clk~CLKOUT~190~1              	190	0	6
twd_clk~CLKOUT~191~1              	191	0	6
clk_25m~CLKOUT~202~1              	202	0	6
sys_clk~CLKOUT~203~1              	203	0	6
o_dq_lo[7]                        	209	0	0
o_dq_hi[7]                        	209	0	2
o_dq_lo[4]                        	211	0	0
o_dq_hi[4]                        	212	0	0
o_dq_oe[7]                        	212	0	2
i_dq_lo[7]                        	212	0	3
o_dq_oe[4]                        	213	0	0
i_dq_lo[4]                        	213	0	1
i_dq_hi[7]                        	213	0	3
i_dq_hi[4]                        	214	0	1
system_gpio_0_io_read[2]          	219	3	3
system_gpio_0_io_write[2]         	219	4	0
system_gpio_0_io_writeEnable[2]   	219	4	2
nrst                              	219	4	3
system_gpio_0_io_read[3]          	219	6	3
system_gpio_0_io_write[3]         	219	8	2
system_gpio_0_io_writeEnable[3]   	219	9	0
system_gpio_0_io_read[1]          	219	9	3
system_gpio_0_io_read[0]          	219	10	3
SYS_PLL_LOCK                      	219	11	1
system_gpio_0_io_write[1]         	219	13	0
system_gpio_0_io_writeEnable[1]   	219	13	2
system_gpio_0_io_write[0]         	219	14	2
system_gpio_0_io_writeEnable[0]   	219	15	0
SYS_PLL_RSTN                      	219	15	2
addr[4]                           	219	23	2
tdqss_clk~CLKOUT~218~25           	219	25	6
addr[8]                           	219	26	0
tdqss_clk~CLKOUT~218~26           	219	26	6
addr[12]                          	219	34	2
tdqss_clk~CLKOUT~218~35           	219	35	6
tdqss_clk~CLKOUT~218~36           	219	36	6
addr[1]                           	219	37	0
ras                               	219	47	2
tdqss_clk~CLKOUT~218~48           	219	48	6
tdqss_clk~CLKOUT~218~49           	219	49	6
cas                               	219	50	0
addr[6]                           	219	58	2
tdqss_clk~CLKOUT~218~59           	219	59	6
tdqss_clk~CLKOUT~218~60           	219	60	6
addr[11]                          	219	61	0
addr[2]                           	219	69	2
tdqss_clk~CLKOUT~218~70           	219	70	6
tdqss_clk~CLKOUT~218~71           	219	71	6
addr[13]                          	219	72	0
we                                	219	93	0
tdqss_clk~CLKOUT~218~94           	219	94	6
addr[0]                           	219	95	2
tdqss_clk~CLKOUT~218~95           	219	95	6
addr[14]                          	219	107	2
tdqss_clk~CLKOUT~218~109          	219	109	6
addr[7]                           	219	110	0
tdqss_clk~CLKOUT~218~110          	219	110	6
ba[2]                             	219	125	0
tdqss_clk~CLKOUT~218~126          	219	126	6
addr[9]                           	219	127	2
tdqss_clk~CLKOUT~218~127          	219	127	6
addr[10]                          	219	137	0
tdqss_clk~CLKOUT~218~138          	219	138	6
reset                             	219	139	2
tdqss_clk~CLKOUT~218~139          	219	139	6
tdqss_clk~CLKOUT~218~149          	219	149	6
i_sysclk_div_2                    	219	159	1
fb                                	219	162	1
clk_125m                          	219	164	1
tmds_tx_clk_TX_DATA[0]            	219	167	0
tmds_tx_clk_TX_DATA[2]            	219	167	2
tmds_tx_clk_TX_DATA[4]            	219	168	0
tmds_tx_clk_TX_DATA[6]            	219	168	2
tmds_tx_clk_TX_DATA[8]            	219	169	0
tmds_tx_clk_TX_DATA[1]            	219	169	2
tmds_tx_clk_TX_DATA[3]            	219	170	0
tmds_tx_clk_TX_DATA[5]            	219	170	2
tmds_tx_clk_TX_DATA[7]            	219	171	0
tmds_tx_clk_TX_DATA[9]            	219	171	2
hdmi_tx_fast_clk~CLKOUT~218~171   	219	171	6
tmds_tx_clk_TX_OE                 	219	173	2
hdmi_tx_slow_clk~CLKOUT~218~175   	219	175	6
tmds_tx_clk_TX_RST                	219	177	0
tmds_tx_data1_TX_DATA[0]          	219	179	0
tmds_tx_data1_TX_DATA[2]          	219	179	2
tmds_tx_data1_TX_DATA[4]          	219	180	0
tmds_tx_data1_TX_DATA[6]          	219	180	2
tmds_tx_data1_TX_DATA[8]          	219	181	0
tmds_tx_data1_TX_DATA[1]          	219	181	2
tmds_tx_data1_TX_DATA[3]          	219	182	0
tmds_tx_data1_TX_DATA[5]          	219	182	2
tmds_tx_data1_TX_DATA[7]          	219	183	0
tmds_tx_data1_TX_DATA[9]          	219	183	2
hdmi_tx_fast_clk~CLKOUT~218~183   	219	183	6
tmds_tx_data1_TX_OE               	219	185	2
hdmi_tx_slow_clk~CLKOUT~218~187   	219	187	6
tmds_tx_data1_TX_RST              	219	189	0
tmds_tx_data2_TX_DATA[0]          	219	192	2
tmds_tx_data2_TX_DATA[2]          	219	193	0
tmds_tx_data2_TX_DATA[4]          	219	193	2
tmds_tx_data2_TX_DATA[6]          	219	194	0
tmds_tx_data2_TX_DATA[8]          	219	194	2
tmds_tx_data2_TX_DATA[1]          	219	195	0
tmds_tx_data2_TX_DATA[3]          	219	195	2
tmds_tx_data2_TX_DATA[5]          	219	196	0
tmds_tx_data2_TX_DATA[7]          	219	196	2
hdmi_tx_fast_clk~CLKOUT~218~196   	219	196	6
tmds_tx_data2_TX_DATA[9]          	219	197	0
tmds_tx_data2_TX_OE               	219	199	0
hdmi_tx_slow_clk~CLKOUT~218~200   	219	200	6
tmds_tx_data2_TX_RST              	219	204	2
tmds_tx_data0_TX_DATA[0]          	219	207	0
tmds_tx_data0_TX_DATA[2]          	219	207	2
tmds_tx_data0_TX_DATA[4]          	219	208	0
tmds_tx_data0_TX_DATA[6]          	219	208	2
tmds_tx_data0_TX_DATA[8]          	219	209	0
tmds_tx_data0_TX_DATA[1]          	219	209	2
tmds_tx_data0_TX_DATA[3]          	219	210	0
tmds_tx_data0_TX_DATA[5]          	219	210	2
tmds_tx_data0_TX_DATA[7]          	219	211	0
tmds_tx_data0_TX_DATA[9]          	219	211	2
hdmi_tx_fast_clk~CLKOUT~218~211   	219	211	6
tmds_tx_data0_TX_OE               	219	213	2
hdmi_tx_slow_clk~CLKOUT~218~215   	219	215	6
tmds_tx_data0_TX_RST              	219	217	0
FPGA_HDMI_SCL_OUT                 	219	223	2
FPGA_HDMI_SDA_OUT                 	219	226	0
FPGA_HDMI_SCL_OE                  	219	226	2
FPGA_HDMI_SDA_OE                  	219	227	0
FPGA_HDMI_SCL_IN                  	219	227	3
FPGA_HDMI_SDA_IN                  	219	228	1
HPD_N                             	219	247	2
HDMI_5V_N                         	219	249	1
hdmi_rx_d0_RX_DATA[0]             	219	254	1
hdmi_rx_d0_RX_DATA[1]             	219	254	3
hdmi_rx_fast_clk~CLKOUT~218~254   	219	254	6
hdmi_rx_d0_RX_DATA[2]             	219	255	1
hdmi_rx_d0_RX_DATA[3]             	219	255	3
hdmi_rx_d0_RX_DATA[4]             	219	256	1
hdmi_rx_d0_RX_DATA[5]             	219	256	3
hdmi_rx_d0_RX_DATA[6]             	219	257	1
hdmi_rx_d0_RX_DATA[7]             	219	257	3
hdmi_rx_d0_RX_DATA[8]             	219	258	1
hdmi_rx_d0_RX_DATA[9]             	219	258	3
hdmi_rx_slow_clk~CLKOUT~218~258   	219	258	6
hdmi_rx_d0_RX_ENA                 	219	260	0
hdmi_rx_d0_RX_RST                 	219	263	0
hdmi_rx_d2_RX_DATA[0]             	219	265	3
hdmi_rx_d2_RX_DATA[1]             	219	266	1
hdmi_rx_d2_RX_DATA[2]             	219	266	3
hdmi_rx_d2_RX_DATA[3]             	219	267	1
hdmi_rx_d2_RX_DATA[4]             	219	267	3
hdmi_rx_fast_clk~CLKOUT~218~267   	219	267	6
hdmi_rx_d2_RX_DATA[5]             	219	268	1
hdmi_rx_d2_RX_DATA[6]             	219	268	3
hdmi_rx_d2_RX_DATA[7]             	219	269	1
hdmi_rx_d2_RX_DATA[8]             	219	269	3
hdmi_rx_d2_RX_DATA[9]             	219	270	1
hdmi_rx_d2_RX_ENA                 	219	271	2
hdmi_rx_slow_clk~CLKOUT~218~271   	219	271	6
hdmi_rx_d2_RX_RST                 	219	274	2
hdmi_rx_d1_RX_DATA[0]             	219	276	1
hdmi_rx_d1_RX_DATA[1]             	219	276	3
hdmi_rx_d1_RX_DATA[2]             	219	277	1
hdmi_rx_d1_RX_DATA[3]             	219	277	3
hdmi_rx_fast_clk~CLKOUT~218~277   	219	277	6
hdmi_rx_d1_RX_DATA[4]             	219	278	1
hdmi_rx_d1_RX_DATA[5]             	219	278	3
hdmi_rx_d1_RX_DATA[6]             	219	279	1
hdmi_rx_d1_RX_DATA[7]             	219	279	3
hdmi_rx_d1_RX_DATA[8]             	219	280	1
hdmi_rx_d1_RX_DATA[9]             	219	280	3
hdmi_rx_slow_clk~CLKOUT~218~283   	219	283	6
hdmi_rx_d1_RX_ENA                 	219	284	0
hdmi_rx_d1_RX_RST                 	219	287	0
hdmi_rx_clk_RX_ENA                	219	298	0
hdmi_rx_clk_RX_DATA               	219	298	1
hdmi_rx_pll_RSTN                  	219	302	2
fb~CLKOUT~218~302                 	219	302	6
system_uart_0_io_txd              	219	310	2
hdmi_rx_pll_LOCKED                	219	312	1
